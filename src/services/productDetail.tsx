...
const getProductImagesPaths = (data = {}, size = '1000x1000') => {
  ...
  } else if (has(data, `main_pair.icons.${size}.image_path`)) {
  ...
  values(data.image_pairs).forEach(img => {
    ...
    } else if (has(img, `icons.${size}.image_path`)) {
...
const convertProductVariants = oldProductVariants => {
  ...
  if (oldProductVariants) {
    const newProductVariants = Object.keys(oldProductVariants).map(variant => {
      const newProductVariant = { ...oldProductVariants[variant] }
      newProductVariant.selectTitle = oldProductVariants[variant].description
      newProductVariant.selectDefaultId = oldProductVariants[variant].variant_id
      ...
      newProductVariant.selectVariants =
        newProductVariant.selectVariants.filter(
          selectVariant =>
            !('disabled' in selectVariant.product) ||
            ('disabled' in selectVariant.product &&
              !selectVariant.product.disabled)
        )
