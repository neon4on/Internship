import { AuthState, CartActionTypes, Coupons } from '../cartTypes'
import {
  CART_SUCCESS,
  CART_FAIL,
...
export default function (state = initialState, action: CartActionTypes) {
  switch (action.type) {
    case RESTORE_STATE:
      return {
        ...state,
    ...
    case CART_RECALCULATE_SUCCESS:
      ...
      newCoupons = JSON.parse(JSON.stringify(state.coupons))
    ...
    case CHANGE_AMOUNT:
      ...
      return {
        ...state,
        carts: JSON.parse(JSON.stringify(newState)),
        fetching: false
      }
