...
export function recalculateTotal(
  ...
  const shippingIds = ids ? Object.values(ids) : []
      ...
      .then(response => {
        ...
        if (response.data.message) {
          notificationsActions.show({
            type: 'warning',
            title: i18n.t('Notice'),
            text: stripTags(response.data.message)
          })(dispatch)
        }
  ...
  return async (dispatch: Dispatch<CartActionTypes>) => {
    ...
    if (Object.keys(response.coupons).includes(coupon.toLowerCase())) {
      ...
      notificationsActions.show({
        ...
        title: i18n.t('Notice'),
        text: i18n.t(
          "Couldn't apply the entered coupon. Either it doesn't meet the requirements, or another coupon code has already been applied."
        )
      })(dispatch)
    }
  }
